select *,
	case 
		when (week_2_new_modules+week_3_new_modules+week_4_new_modules)>3 THEN 'SPRINTER'
		when (week_2_new_modules+week_3_new_modules+week_4_new_modules)=3 then 'CRUISER'
		when (week_2_new_modules+week_3_new_modules+week_4_new_modules)<3 AND (week_2_login+week_3_login+week_4_login)>=1 then 'TRAILER'
		when (week_2_new_modules+week_3_new_modules+week_4_new_modules)=0 AND (week_2_login+week_3_login+week_4_login)=0 then 'SLUGGER'
	end as Segment,
	case
		when week_1_new_modules>1 then 'SPRINTING'
		when week_1_new_modules=1 then 'CRUISING'
		when week_1_new_modules<1 AND week_1_login=1 then 'TRAILING'
		when week_1_new_modules=0 AND week_1_login=0 then 'SLUGGING'
	end as Status
from
(select 
...
from aprende.cohort_week_diplomado
	...
	where ( moodle_user_course_completion_summary.general_sequence = 1)
			and datediff(day,contact_creation_week,dateadd(day,-1,date_trunc('week',current_date)))>=35) as tbl1 where tbl1.ranking between 1 and 5 and tbl1.ranking_login between 1 and 5
group by 
tbl1.contact_creation_week,
tbl1.student_id,
tbl1.email,
...
tbl1.school,
tbl1.diplomado,
tbl1.content_course_version) where segment is null order by contact_creation_week DESC;

select student_id,moodle_course_id,week_info,has_login from cohort_week_diplomado where moodle_course_id=405 and student_id='0035G00001vUSawQAG' order by week_info desc;


--
--
--
SELECT cohort_week_diplomado.contact_creation_week,
               cohort_week_diplomado.student_id,
               cohort_week_diplomado.moodle_course_id,
               cohort_week_diplomado.week_info 
from aprende.cohort_week_diplomado
 where cohort_week_diplomado.student_id='0035G00001oh2gbQAA';
...
select datediff(day,contact_creation_week,dateadd(day,-1,date_trunc('week',current_date))) as prueba,
dateadd(day,-1,date_trunc('week',current_date)),contact_creation_week from cohort_week_diplomado order by contact_creation_week desc;

select contact_creation_week from cohort_week_diplomado order by contact_creation_week desc;
